...
This package distributes demand data from the São Paulo metro system in
R. The data covers 2012 to 2026 and comes from several operators. Most
of it is already
[public](https://transparencia.metrosp.com.br/dataset/demanda), but it’s
scattered across poorly structured CSV and PDF files.

The data is cleaned and standardized to make it easier to work with. The
datasets are updated weekly and are accessible through
`read_metro_demand()`.
...
The core demand datasets are listed below. The package also bundles
auxiliary datasets, such as `stations` and `lines`, which provide
spatial datasets for the stations and metro lines.
...
Most users should start with `read_metro_demand()`, which loads the most
recent demand data. Every pipeline run publishes a fresh build of the
four demand datasets to the [`data-latest`
release](https://github.com/viniciusoike/metrosp/releases), plus a dated
tag that pins that month’s batch. Downloads go to a temporary directory
unless the user allows a persistent cache with `metrosp_cache_enable()`.
...
The package also includes a fixed, bundled snapshot with observations
through July 2026. It works offline and is useful for examples across
packages. The snapshot will not be updated unless METRÔ revises past
values.
