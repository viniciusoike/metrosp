...
Flags are env vars, replacing the old `run_pipeline.R` booleans. Each
gates a side-effecting refresh that rewrites the committed CSVs in
place; unset (default off) means rebuild offline from those CSVs:
