...
**Files** in `data-raw/R/` are named by source, not by dataset:
- `import_metro_current.R` — current-era (2020-) METRO portal
- `import_historic.R` — 2016-2019 METRO portal
- `import_dataverse.R` — Lines 4/5 (Insper Dataverse)
- `import_geosampa.R` — spatial data
- Derived datasets use a `build_` prefix (`build_calendar_spo.R`, `build_inauguration.R`)

The retired `import_{dataset}[_{period}].R` convention survives only in
`data-raw/archive/`, which holds the pre-`targets` scripts.

**Functions** are `<verb>_<noun>[_<era>]`. The verb says where in the chain it sits:

| Verb | Does |
|---|---|
| `read_*` | Parse one raw file into a wide frame. No reshaping. |
| `clean_*` | Reshape one wide frame into the long import schema. |
| `import_*` | `read_*` + `clean_*` for one unit (one file, or one year of files). |
| `build_*` | Loop `import_*` over a period; returns a whole table. |
| `refresh_*` | The gated side effect: rewrite a committed `processed/*.csv`. |
| `assemble_*` | Harmonize the eras into one exported dataset. |
| `get_*` | Look up a path or metadata. |

The nouns are the pipeline's three measures, abbreviated: `psg_line` (passengers
by line), `stn_avg` (weekday station averages), `stn_daily` (daily station
entries). Era suffixes are `_current` (2020-), `_monthly` (the 2017-2019
per-month files), `_2016`, `_2017_pdf`, `_4_5`.

**No `.` prefix on functions.** `tar_source()` loads every file into one
environment, so names must be globally unique and the era suffix is what does
that. The dot is reserved for non-function constants (`.cols_*`,
`.footnote_pattern`, `.current_csv_names`) — there it marks "not callable".
Two names to avoid when adding helpers: `near` and `package_version` shadow
`dplyr::near()` and `base::package_version()`.

Readers shared by more than one era live in `helpers.R`; readers specific to one
era live in that era's `import_*.R`.
...
- **Never hardcode a row offset into a reader.** The METRO exports move their
  headers (2025 and 2026 each added rows) and change their line roster, and a
  stale offset table fails silently rather than erroring. Locate blocks by
  scanning the file's own text — `^LINHA`, `^Total`, `^DEMANDA`, `^DIA;`,
  `^Jan;` — and take the line roster from the header row via
  `split_line_labels()` / `label_line_number()` in `helpers.R`.
- **`dim_line` is joined into the exported `lines` dataset.** Adding a column to
  it leaks into package data; `build_geosampa()` joins `select(dim_line, -line_name_full)`
  for that reason. The `schema_ok` target catches the leak, so heed it rather
  than refreezing.
...
unconditionally, which the frozen-snapshot model forbids. The `import_*.R`,
`make_datasets.R`, `utils.R`, and `download_metro.R` scripts it drove now sit in
`data-raw/archive/`. Nothing sources them, and the live versions of the ones
that still have a counterpart are in `data-raw/R/`.
